/*
 * scull_pipe.c -- fifo driver for scull
 *
                ...
                loff_t *f_pos)
{
	...
	while (dev->rp == dev->wp) { /* nothing to read */
		...
		PDEBUG("\"%s\" reading: ready to awake\n", current->comm);
		if (wait_event_interruptible(dev->inq, (dev->rp != dev->wp)))
		/* otherwise loop, but first reacquire the lock */
		if (down_interruptible(&dev->sem))
			return -ERESTARTSYS;
	}
    PDEBUG("\"%s\" reading: I'm really awake\n", current->comm);
...
static void scull_p_setup_cdev(struct scull_pipe *dev, int index)
{
	...
	if (err){ /* Fail gracefully if need be */
        ...
        printk(KERN_ALERT "ADding scullpipe%d successed!\n", index);
    }
}
...
#ifdef SCULL_DEBUG
	create_proc_read_entry("scullpipemem", 0, NULL, scull_read_p_mem, NULL);
#endif
...
void scull_p_cleanup(void)
{
	...
	for (i = 0; i < scull_p_nr_devs; i++) {
        ...
        printk(KERN_ALERT "Deleting scullpipe%d succeed!\n", i);
	}
